# Appendix I: Preparation of processed bacterial read abundance data and metadata
Roxana J. Hickey, with contributions from Matthew L. Settles  

### Description
This script works through the preparation of bacterial read abundance data (from Roche 454 pyrosequencing) and participant metadata for subsequent analysis. It also generates the custom color palettes used to make figures for downstream analyses. See the paper "Vaginal microbiota of adolescent girls resemble those of reproductive-age women prior to the onset of menarche" by Hickey et al. for information about the objectives and study design.

The analyses can be run directly from the R Markdown file "01-data-prep.Rmd" using RStudio. The data files necessary to run the script are located at [github address]. Before starting, you will want to check that the necessary packages are installed (see the library calls in the first section).

Authors: Roxana J. Hickey <roxana.hickey@gmail.com> and Matthew L. Settles <mattsettles@gmail.com>

*Disclaimer: Please excuse any clunky code, but if you identify any errors please feel free to notify the author(s).*
...
# Part I: processing of bacterial read abundance data
...
### Read in bacterial read data and whip into shape
Sequence data were generated by Roche 454 pyrosequencing and summarized into taxon abundance tables following preprocessing steps outlined in the manuscript and supplementary materials. At this stage we input the phylotype abundance data. Most taxa are assigned to the genus level or higher, with the exception of *Gardnerella*, *Lactobacillus* and *Streptococcus*, which were speciated using the PATRIC database and a dynamic tree-cutting algorithm in WGCNA to cluster 16S V1-V3 sequences. This resulted in many hits to known species, but also a lot of "species" clusters that did not have a named top hit. These got written into the species abundance table as things like "Lactobacillus c.1", "Lactobacillus c.2" and so forth. These may represent different strains or "ribotypes", but many (especially the low-abundance ones) could simply be artifacts of differences in 454 read length. Since we aren't really interested in these for the purpose of our analysis anyway, we will collapse them back to the genus level and retain only the named species. To do this we'll extract the species we want from the species abundance table, substract their totals from the appropriate genus in the genus abundance table, then combine them into one table. We'll check total read counts as we move along to ensure we don't mistakenly add or lose anything in the process.
...
## found in the vagina, so we'll select those names over the rest. For all 
...
### Check read counts
Next we'll check read counts and discard any samples that are too low. I was fairly liberal and tossed only one sample that had just one read.
...
hist(colSums(abund), breaks=50, xlab="Number of Reads", 
     main="Histogram of Sample Read Counts", col="turquoise3")
```
...
### Slimming down the abundance table
Now we'll reduce the read abundance table to a more manageable size, keeping only the most common taxa. The rest will be grouped in the "Other Bacteria" category. We're not discarding anything, just regrouping.
...
# Part II: processing of participant metadata
...
* tan.br.dr = physician-assessed Tanner breast score (1-5 discrete)
* tan.gen.dr = physician-assessed Tannger genital score (1-5 discrete)
...
### Read in metadata and whip into shape
...
## Make "full" metadata object with vagina and vulva samples expanded
...
# Part III: set custom color palettes and functions for downstream figure generation
...
```r
## First, define a palette of 50 colors laboriously hand-selected by the author
...
pie(rep(1,50), main="Palette: col.50 (n=50)", 
    col=col.50, labels=1:50)
pie(rep(1,31), main="Palette: col.gm.pair (n=31)", 
    col=col.gm.pair, labels=unique(meta$gm.pair))
pie(rep(1,2), main="Palette: col.type (n=2)", 
    col=col.type, labels=names(col.type))
pie(rep(1,2), main="Palette: col.site (n=2)", 
    col=col.site, labels=names(col.site))
pie(rep(1,4), main="Palette: col.men.stat (n=4)", 
    col=col.men.stat, labels=names(col.men.stat))
pie(rep(1,6), main="Palette: col.tanner (n=6)", 
    col=col.tanner, labels=names(col.tanner))
pie(rep(1,4), main="Palette: col.nugent (n=4)", 
    col=unique(col.nugent), 
    labels=c("NA","Low (0-3)", "Med (4-6)", "High (7-10)"))

## Combine into metadata color palette object (full 508 rows)
...
## Define (semi-custom) colors for 99 taxa
## To be improved upon 6/24/14
...
![plot of chunk color-palettes](./01-data-prep_files/figure-html/color-palettes.png) 

### Source custom ggplot themes and functions
...
### Save R data in data directory for easy loading next time (file name appended with current date)
This will save the data in two separate images -- one named with today's date, in case you ever need to restore that version, and another with a non-dated name that can be easily loaded into subsequent analyses.
