...
```
## Warning: 'figures' already exists
```
...
```
## Warning: 'supplemental' already exists
```
...
abund.g <- read.table("data/bacterial-composition/genus-abund.txt", 
...
abund.s <- read.table("data/bacterial-composition/species-abund.txt", 
                      sep="\t", header=T, row.names=1, as.is=T)
da.s <- dim(abund.s)
...
```r
## Input read counts
readData <- read.table("data/bacterial-composition/read-counts.txt", 
                       sep="\t", header=TRUE)
...
```r
source("./data/custom-ggplot-settings.R") # custom ggplot settings
source("./data/heatmap.3.rjh.R") # modified from heatmap.3 (see file for reference)
source("./data/hcoplot.R") # from Borcard, Gillet and Legendre (2009) Numerical Ecology with R. Springer.
```
