...
```{r fig-1-hclust-heatmap, fig.width=12, fig.height=8, echo=FALSE}
...
colnames(fcol) <- c("Menarche Status", "Girl/Mother", "Cluster Group")
...
legend(0.15, -0.085, legend=sort(names(col.hclust.vag)), title="Cluster Group", 
...
```{r hclust-count-prop, echo=FALSE}
...
```{r fig-2-hclust-sampletype, fig.width=8, fig.height=5, echo=FALSE}
## Uncomment the next line to save as a PDF
...
```{r fig-3-hclust-over-time, fig.width=8, fig.height=5, echo=FALSE}
...
```{r fig-s3-pcoa, fig.width=12, fig.height=6, echo=FALSE}
...
# Fig. S3A
## Uncomment the next line to save as a PDF
# pdf("supplemental/fig-s3-pcoa.pdf", width=8, height=4, pointsize=8)
par(mfrow=c(1,2))
...
mtext("A", side=3, line=1, adj=-0.1, font=2, cex=2)
...
legend("bottomright", bty="n", title="Sample Type",
       legend=c("Pre", "Post", "Mother"), 
       pch=c(1,16,17))
mtext("B", side=3, line=1, adj=-0.1, font=2, cex=2)
dev.off()
```
...
```{r pcoa-extra-subjects, fig.width=7, fig.height=7, echo=FALSE}
## Uncomment the next line to save as a PDF
...
```{r pcoa-extra-race, fig.width=7, fig.height=7, echo=FALSE}
