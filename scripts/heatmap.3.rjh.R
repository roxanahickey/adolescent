### Originally "heatmap.microbe.R" by Matt Settles
### Edited by RJH following heatmap.3.R referenced here: http://www.biostars.org/p/18211/
...
heatmap.3 <- function (x, 
                       ...
                       labCol = NULL,
                       key = TRUE,
                       keysize = 0.5, # heatmap.3.R uses 1.5
                       dendrosize=1.5, 
...
### trying to debug
#      axis(1, 1:(dim(rsc)[2] - 1)/(dim(rsc)[2] - 1), rownames(RowSideColors), las = 2, tick = FALSE)
    }
  }
}
...
# axis(4, iy, labels = labRow, las = 2, line = -0.5, tick = 0, font=3,
#      cex.axis = cexRow)
axis(2, iy, labels = labRow, las = 2, line = -0.5, tick = 0, font=3,
     cex.axis = cexRow)
if (!is.null(ylab))
...
## Plot KEY
if (key) {
  par(mar = c(4, 2, 2, margins[2]),cex=0.7) # original heatmap.microbe mar=c(4,0,0...)
